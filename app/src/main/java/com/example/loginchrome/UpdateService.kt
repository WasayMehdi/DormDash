...
class UpdateService: Service() {
    ...
    companion object {
        ...
        const val NOTIFICATIONCOL = "notifications";
    ...
    override fun onCreate() {
        super.onCreate()
        Log.e(ViewOrdersFragment.TAG, "STARTING THE SERVICE");
        isRunning = true;
        if(db == null)
        ...
        if (account != null) {
            ...
            notificationEvent = db?.collection(NOTIFICATIONCOL)?.whereEqualTo("emailTo", account.email)
    ...
    fun startListener(model: OrderViewModel?) {
        ...
        updateEvent?.remove();
        updateEvent = model?.db?.collection(ORDERSCOL)
        ...
        notificationEvent?.remove();
        notificationEvent = model?.db?.collection(NOTIFICATIONCOL)?.whereEqualTo("emailTo", model?.email)?.addSnapshotListener { querySnapshot, firebaseFirestoreException ->
            notificationHandling(querySnapshot)
